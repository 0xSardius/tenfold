...
| Your auth endpoint, e.g. `/api/auth` (if applicable) | Your app's custom auth endpoint, called from within the Farcaster client. [Quick Auth](/docs/sdk/quick-auth) uses `https://auth.farcaster.xyz` by default and doesn't call your domain — only needed if you have a custom auth endpoint. |
...
Only needed if you have a custom auth endpoint (not a Farcaster-defined route).
Replace the path with whatever your app uses. Consider scoping with a
User-Agent condition (e.g. User Agent contains `farcaster`) to narrow the
bypass surface.
